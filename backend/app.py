...
class RateLimiter:
    ...
    def check_ai_rate_limit(self, email, max_requests=10, window=60):
        # Specific rate limit for AI calls
        with self._lock:
            now = time.time()
            if "ai" not in self._email_requests:
                self._email_requests["ai"] = {}
            
            store = self._email_requests["ai"]
            if email not in store:
                store[email] = {"count": 0, "last": now}
            
            if now - store[email]["last"] > window:
                store[email] = {"count": 1, "last": now}
            else:
                store[email]["count"] += 1
            
            if store[email]["count"] > max_requests:
                return False
        return True
...
def ai_translate():
    ...
    if not text or not user_email:
        return jsonify({"error": "Text and userEmail are required"}), 400

    # Rate limiting for AI calls
    if not rate_limiter.check_ai_rate_limit(user_email):
        return jsonify({"error": "AIリクエストの回数制限を超えました。少し待ってから再試行してください。"}), 429

    api_key = os.environ.get("NVIDIA_NIM_API_KEY")
    if not api_key or api_key == "your-nvidia-nim-api-key-here":
        # Fallback message if API key is not set
        return jsonify({"error": "AI翻訳機能は現在設定されていません。管理者に連絡してください。"}), 501

    try:
        # Prompt construction for translation
        prompt = f"Translate the following text to {target_lang}. Provide only the translation, no extra text: {text}"
        ...
        app.logger.error(f"AI Translation Error: {e}")
        return jsonify({"error": "AI翻訳中にエラーが発生しました"}), 500


@app.route("/api/debug", methods=["POST"])
def debug_endpoint():
    return jsonify({"raw": request.get_data(as_text=True)})
