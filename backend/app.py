    ...
    def record_login_failure(self, identifier, is_ip=True):
        with self._lock:
            ...
            if self._login_attempts[key]['count'] >= 5:
                block_duration = min(300 * (2 ** (self._login_attempts[key]['count'] - 5)), 3600)
                if is_ip:
...
class CSRFProtector:
    def __init__(self, secret_key=None):
        self._secret_key = secret_key or os.environ.get('SECRET_KEY', secrets.token_hex(32))
...
def csrf_protected(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        csrf_token = request.headers.get('X-CSRF-Token') or (request.json.get('csrfToken') if request.is_json and request.json else None)
        
        if not csrf_token:
            app.logger.warning(f"CSRF validation failed: token missing for {request.path}")
        ...
        if not user_identifier:
            app.logger.warning(f"CSRF validation failed: user_identifier missing for {request.path}")
            return jsonify({"error": "User identifier missing for CSRF validation"}), 403
        
        is_valid = csrf_protector.validate_token(user_identifier, csrf_token)
        if not is_valid:
            app.logger.warning(f"CSRF validation failed: invalid token for user={user_identifier}, path={request.path}")
...
def init_db():
    ...
    with conn.cursor() as c:
        c.execute('''CREATE TABLE IF NOT EXISTS users (email VARCHAR(255) PRIMARY KEY, username VARCHAR(255), password VARCHAR(255))''')
        c.execute('''CREATE TABLE IF NOT EXISTS verification_codes (email VARCHAR(255) PRIMARY KEY, code VARCHAR(255))''')
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS folder_likes (user_email VARCHAR(255), folder_id INT, PRIMARY KEY(user_email, folder_id))''')
        c.execute('''CREATE TABLE IF NOT EXISTS folder_favorites (user_email VARCHAR(255), folder_id INT, PRIMARY KEY(user_email, folder_id))''')
    conn.commit()
    conn.close()
...
def login():
    ...
    if rate_limiter.is_ip_blocked(client_ip):
        return jsonify({"message": "一時的にブロックされました。しばらくしてから再試行してください。"}), 429
    
    allowed, _ = rate_limiter.check_ip_rate_limit(client_ip, max_requests=30, window=60)
    if not allowed:
        return jsonify({"message": "リクエストが多すぎます。しばらくしてから再試行してください。"}), 429
    ...
    conn = get_db()
    with conn.cursor() as c:
        c.execute('SELECT * FROM users WHERE email = %s OR username = %s', (login_id, login_id))
        user = c.fetchone()
    conn.close()
    ...
    if block_duration > 0:
        return jsonify({"message": f"ログイン試行回数が多すぎます。{int(block_duration)}秒後に再試行してください。"}), 429
...
def signup():
    ...
    allowed, retry_after = rate_limiter.check_email_rate_limit(email, max_requests=5, window=300)
    if not allowed:
        return jsonify({"message": f"リクエストが多すぎます。{retry_after}秒後に再試行してください。"}), 429
    ...
    try:
        c.execute('INSERT INTO users (email, username, password) VALUES (%s, %s, %s)', 
...
def send_code():
    ...
    allowed, retry_after = rate_limiter.check_email_rate_limit(email, max_requests=3, window=300)
    if not allowed:
        return jsonify({"message": f"このメールアドレスからのリクエストが多すぎます。{retry_after}秒後に再試行してください。"}), 429
    
    code = str(random.randint(100000, 999999))
    
    conn = get_db()
    c = conn.cursor()
    c.execute('REPLACE INTO verification_codes (email, code) VALUES (%s, %s)', (email, code))
    conn.commit()
    conn.close()
...
def create_folder():
    ...
    conn = get_db()
    with conn.cursor() as c:
        c.execute('INSERT INTO folders (user_email, title, visibility) VALUES (%s, %s, %s)',
...
def save_cards():
    try:
        ...
        conn = get_db()
        with conn.cursor() as c:
            c.execute("SELECT email FROM users WHERE email = %s OR username = %s", (login_id, login_id))
            user_row = c.fetchone()
            ...
            for idx, card in enumerate(cards_data):
                ...
                c.execute('INSERT INTO cards (folder_id, order_index, front_content, back_content, front_bg, back_bg) VALUES (%s, %s, %s, %s, %s, %s)',
...
def list_folders():
    ...
    conn = get_db()
    with conn.cursor() as c:
        c.execute('SELECT id, title, visibility, likes FROM folders WHERE user_email = %s', (user_email,))
        folders = c.fetchall()
    conn.close()
    ...
    try:
        ...
        with conn.cursor() as c:
            ...
            if folder['visibility'] != 'public' and folder['user_email'] != user_email:
                conn.close()
                return jsonify({"message": "このフォルダにアクセスする権限がありません"}), 403
            
            c.execute('SELECT front_content, back_content, front_bg, back_bg FROM cards WHERE folder_id = %s ORDER BY order_index', (folder_id,))
            rows = c.fetchall()
        conn.close()
        ...
        for row in rows:
            ...
            if front and front.startswith('"') and front.endswith('"'):
                try: front = json.loads(front)
                except: pass
            if back and back.startswith('"') and back.endswith('"'):
                try: back = json.loads(back)
                except: pass
...
def update_folder():
    ...
    with conn.cursor() as c:
        ...
        c.execute('UPDATE folders SET title = %s, visibility = %s WHERE id = %s', 
...
def get_folders():
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT email FROM users WHERE email = %s OR username = %s", (login_id, login_id))
        ...
        return jsonify({"folders": folders, "totalPages": total_pages, "currentPage": page}), 200
...
def toggle_action():
    ...
    with conn.cursor() as c:
        # すでに登録されているか確認
        c.execute(f'SELECT 1 FROM {table} WHERE user_email = %s AND folder_id = %s', (user_email, folder_id))
        if c.fetchone():
            # あれば削除（取り消し）
            c.execute(f'DELETE FROM {table} WHERE user_email = %s AND folder_id = %s', (user_email, folder_id))
        else:
            # なければ追加
            c.execute(f'INSERT INTO {table} (user_email, folder_id) VALUES (%s, %s)', (user_email, folder_id))
    conn.commit()
    conn.close()
    return jsonify({"message": "Success"}), 200
...
def get_user_stats():
    ...
    try:
        ...
        with conn.cursor() as c:
            ...
            user_row = c.fetchone()
            username = user_row['username'] if user_row and user_row['username'] else user_email.split('@')[0]
...
def delete_card():
    ...
    try:
        ...
        with conn.cursor() as c:
            ...
            if c.rowcount == 0:
                conn.close()
                return jsonify({"message": "このカードへのアクセス権限がありません"}), 403
        conn.commit()
        conn.close()
    ...
    with conn.cursor() as c:
        ...
        for user in users:
            ...
            if not old_pass.startswith(('pbkdf2:sha256', 'scrypt:', 'bcrypt:')):
                ...
                c.execute('UPDATE users SET password = %s WHERE email = %s', (new_hash, user['email']))
...
def admin_migrate_passwords():
    admin_key = request.headers.get('X-Admin-Key') or request.json.get('adminKey') if request.is_json else None
